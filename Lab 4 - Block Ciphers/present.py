...
def rol(val, r_bits, max_bits): 
    """
    val : key to be rotated
    r_bits : number of bits to rotate by
    max_bits = number of bits of key
    """
    return \
...
def genRoundKeys(key):
    """
    key = 80 bits
    round i : Ki = κ63κ62 ...κ0 = k79k78 ...k16.
    rotate key left by 61 bits
    pass first 4 bits (bit 79 to 76) through s-box
    XOR bits 19 to 15 with LSB of round_count  
    """
...
def sBoxLayer(state):
    """
    The action of this box in hexadecimal notation
    For sBoxLayer the current state b63 . . . b0 is considered as 
    sixteen 4-bit words w15 ...w0 where 
    wi = b4*i+3||b4*i+2||b4*i+1||b4*i for 0 ≤ i ≤ 15 
    the output nibble S[wi] provides the updated state values
    """
    output = 0
...
def present_round(state, roundKey):
    """
    for each iteration of present, do:
...
def pLayer_inv(state):
    """
    i = new_index after permutation
    i.e at i = 2, find index of 2 from pmt[]
    index from pmt[] = old index 
    """
    output = 0
    for i in range(len(pmt)):
        bit_state = state & 0b1
