...
class Polynomial2:
    ...
    #coefficients given are from lowest power to highest power
    def __init__(self,coeffs):
        self.coeffs = coeffs

    def add(self,p2):
        """
        add coefficients based on index
        assign 0 to coefficients if missing term
        addition = XOR
        """
        output = []
    ...
    def sub(self,p2):
        #Addition and subtraction on polynomials are the same
        return self.add(p2)

    def shift_right(self):
        # add 0 to LSB to simulate right shift
    ...
    def mul(self,p2,modp=None):
        """
        If MSB of previous result = 0, shift the previous result 
        one bit to the left --> i.e multiply the polynomial with an x
        If MSB of previous result = 1,
        1. shift it one bit to the left
        2. XOR it with the modulus, without MSB
        --> i.e if we multiply polynomial with x and result same degree 
        as modulus -> reduce result with irreducible polynomial or modulus
    ...
    def __str__(self): # convert result is string
        """
        index = power, value = coefficient
        GF(2) --> coefficients are only 0/1
        """
        output = ""
...
class GF2N:
    ...
    def __init__(self,x,n=8,ip=Polynomial2([1,1,0,1,1,0,0,0,1])):
        """
        x = number to be represented in Galois Field
        n = power, --> 2**n
    ...
    def mulInv(self):
        """
        given a, find b such that a*b = 1
        OR : P(x) = Q(x)*mult_inv + remainder % 2

        how : 
        bezout lemma : gcd(a,b) = as + bt
        --> gcd(p,a) = xp + ya = 1

        extended Euclid : 
        if b / a = 1: return b,0,1
        else: 
            a = qb + r
            d,x,y = egcd(b,r)
            return d,y,x-yq
